<?php
	...
	$totalRow = mysql_num_rows(mysql_query("select * from ebooks"));
	
	$currentDateTime = date('y-m-j H-i-s'); 
					
	mysql_query("insert into ebooks values('ebk".($totalRow+1)."','".$_POST['etitle']."','".
	$_POST['eauthor']."','".$_POST['ebelongs']."','".$_FILES["elocation"]["size"]."','".
	$currentDateTime."','".$_GET['uid']."')");
	
	uploadEbook($_FILES["elocation"],"ebk".(string)($totalRow+1).".");
	
	$currentTimeStamp = date('y-m-j H-i-s');
	writeLogFile($currentTimeStamp." : ".$_GET['uid']." uploaded ebooks ebk".($totalRow+1));
?>	
...
<head>
	...
	<title>OES :: Ebook uploaded</title>
