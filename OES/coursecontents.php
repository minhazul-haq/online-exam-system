<?php	
	...
	{
		$result = mysql_query("SELECT * FROM course_contents,courses where belongs_to_course=course_id");
	}
	else
	{
		if (strcmp($_POST['selectoption'],"title")==0)
		{
			$result = mysql_query("SELECT * FROM course_contents,courses ".
			"where content_title like '%".$_POST['searchfield']."%' and belongs_to_course=course_id");
		}
		else if (strcmp($_POST['selectoption'],"course")==0)
		{
			$result = mysql_query("SELECT * FROM course_contents,courses ".
			"where course_no='".$_POST['searchfield']."' and belongs_to_course=course_id");
		}		
	}	
?>
...
<head>
	...
	<title>OES :: Course contents</title>
...
<table width="738" border="0" align="center" cellpadding="1" cellspacing="1" class="tableBackground">
  ...
  <tr>
    <td height="296"><table width="595" border="0" align="center">
	 <form name="frmEBook" method="post" action="coursecontents.php?uid=<?php echo $_GET['uid']; ?>">
      <tr>
    ...
    <table width="900" border="0" align="center">
      <tr bgcolor="#CCCCCC">
        ...
        <td width="150"><div align="left"><strong>Belongs to course </strong></div></td>
	  ...
	  ?>
		<tr>
        	...
        	<td><?php echo (round($row['content_size']/1000))." KB"; ?>&nbsp;</td>
			<td>
			<form name="frmDownload" method="post" action=<?php echo "course_contents/".$row['content_id'].".pdf" ?>>
